...
def p_one_workbook_downloader(p1_workbook_path: str) -> None:
    """
    This function downloads the latest p1 workbook from the outlook mails.
    :param p1_workbook_path: Path of the p1 workbook.
    """
    outlook = win32.Dispatch("Outlook.Application")
    mapi = outlook.GetNamespace("MAPI")
    inbox = mapi.GetDefaultFolder(6)
    messages = inbox.Items
    subject_we_are_looking_for = "MPBN Planning Automation Tracker P1_sheet: "
    messages.Sort("[ReceivedTime]", True)
    message_found = False

    acceptable_mail_received_time = (datetime.now() - timedelta(days=5)).replace(hour=0, minute=0, second=0)
    ...
    while i < len(messages):
        try:
            ...
            if dt >= acceptable_mail_received_time:
                # print(f"{dt = }")
                # print(f"{acceptable_mail_received_time = }")
                # print(f"{message.Subject =}")
                # print(f'{message.Subject.strip().upper().__contains__(subject_we_are_looking_for.upper()) = }')
                if message.Subject.strip().upper().__contains__(subject_we_are_looking_for.upper()):
                    message_found = True
                    attachments = message.Attachments
                    attachment = attachments.Item(1)
                    attachment.SaveAsFile(p1_workbook_path)
                    break
    ...
    if not message_found:
        number_of_subfolders_of_inbox = len(inbox.Folders)

        i = 0
        while i < number_of_subfolders_of_inbox:
            neo_messages = inbox.Folders[i].Items
            neo_messages.Sort("[ReceivedTime]", True)
            # print(f"Folder name ==> {inbox.Folders[i].Name}\n\n")

            if len(neo_messages) > 0:
                j = 0
                while j < len(neo_messages):
                    try:
                        message = neo_messages[j]
                        dt = message.ReceivedTime
                        year, month, day, hour, minute, second = dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second
                        dt = datetime(year=year, month=month, day=day, hour=hour, minute=minute, second=second)
                        if dt >= acceptable_mail_received_time:
                            ...
                            if message.Subject.strip().upper().__contains__(subject_we_are_looking_for.upper()):
                                ...
                                attachment = attachments.Item(1)
                                attachment.SaveAsFile(p1_workbook_path)
                                # print("Saved the file and now breaking out of the loop")
                                break
                            else:
                                j+=1
                                continue
    ...
    if not message_found:
        ...
        i = 0
        while i < number_of_subfolders_of_inbox:
            number_of_subfolders_of_subfolder_of_inbox = len(inbox.Folders[i].Folders)

            j = 0
            while j < number_of_subfolders_of_subfolder_of_inbox:
                # print(f"Folder name ==> {inbox.Folders[i].Folders[j].Name}\n\n")
                neo_neo_messages = inbox.Folders[i].Folders[j].Items
                neo_neo_messages.Sort("[ReceivedTime]", True)
                if len(neo_neo_messages) > 0:
                    k = 0
                    while k < len(neo_neo_messages):
                        try:
                            message = neo_neo_messages[k]
                            dt = message.ReceivedTime
                            year, month, day, hour, minute, second = dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second
                            dt = datetime(year=year, month=month, day=day, hour=hour, minute=minute, second=second)
                            if dt >= acceptable_mail_received_time:
                                ...
                                if message.Subject.strip().upper().__contains__(subject_we_are_looking_for.upper()):
                                    ...
                                    attachment = attachments.Item(1)
                                    attachment.SaveAsFile(p1_workbook_path)
                                    # print("Saved the file and now breaking the loop")
                                    break
    ...
    if not message_found:
        raise CustomException("MPBN Planning Automation Tracker P1_sheet mail Not Found!! ",
                              "No email found with subject 'MPBN Planning Automation Tracker P1_sheet, Kindly Check!'")
...
def p_one_p_three_appender(sender, workbook):
    ...
    if sender not in unique_technical_validator_set:
        ...
        #     neo_response = messagebox.askokcancel("   P1 workbook doesn't exist!",
        #                                           f"'MPBN Planning Automation Tracker P1 Sheet ' doesn't exist. Download the same from outlook (latest mail) and then press ok!")
        #
        #     if (not neo_response):
        ...
        #
        # else:
        #     response = messagebox.askyesno("P1 workbook already exists!",
        #                                    f"'MPBN Planning Automation Tracker P1' already exists.Hope you have downloaded the same from latest mail. If yes then (click 'Yes') or (click 'No') if you want to drop this task?")
        #
        #     if (not response):
        ...
        if Path(p1_workbook_file).exists() == True:
            os.remove(p1_workbook_file)

        # Calling the function to download the workbook from outlook
        p_one_workbook_downloader(p1_workbook_file)

        messagebox.showinfo("   P1 workbook downloaded!", "MPBN Planning Automation Tracker P1 Sheet downloaded. Now, appending today CR details, Please Wait!")
...
def styling(workbook, sheetname):
    ...
    for row in ws:
        for cell in row:  # type: ignore
            ...
            cell.border = Border(top=Side(border_style='medium', color='000000'), bottom=Side(border_style='medium', color='000000'), left=Side(border_style='medium', color='000000'),
...
def paco_cscore(sender, workbook):  # type:ignore
    try:
        ...
        if (len(difference) > 0):
            ...
            if (len(input_error) > 0):
                ...
                input_error.sort()
                # print(input_error)
                raise CustomException("  Input Errors",
                                      f"Input Error in Planning Sheet! Check 'Location', 'Circle', 'Change Responsible', 'Impact', 'Vendor', 'Protocol' & 'Execution Projection' for S.NO.: {','.join([str(num) for num in input_error])}")
            ...
            for i in range(0, len(daily_plan_sheet)):
                if ((daily_plan_sheet.iloc[i]['Domain kpi'].upper() == 'PS-CORE') or (daily_plan_sheet.iloc[i]['Domain kpi'].upper() == 'PS') or (
                        daily_plan_sheet.iloc[i]['Domain kpi'].upper() == 'PS_CORE') or (daily_plan_sheet.iloc[i]['Domain kpi'].upper().startswith('PACO')) or (
                daily_plan_sheet.iloc[i]['Domain kpi'].upper().startswith("PS")) and (daily_plan_sheet.iloc[i]['Planning Status'].upper() == 'PLANNED')):
            ...
            dictionary1 = {'CR': mpbn_cr_no, 'Maintenance Window': maintenance_window, 'CR Category': cr_category, 'Impact': impact, 'Location': location, 'Circle': circle,
                           'MPBN Activity Title': mpbn_activity_title, 'CR Owner Domain': cr_owner_domain, 'Change Responsible': mpbn_change_responsible_executor,
                           'Technical Validator/Team Lead': validator, 'InterDomain': inter_domain, 'Impacted Node Details': impacted_node_details, 'KPIs to be monitored': Kpis_to_be_monitored}
            ...
            Kpis_to_be_monitored = []
            for i in range(0, len(daily_plan_sheet)):
                if ((daily_plan_sheet.iloc[i]['Domain kpi'].upper().startswith("CS")) or (daily_plan_sheet.iloc[i]['Domain kpi'].upper().startswith("STP")) or (
                daily_plan_sheet.iloc[i]['Domain kpi'].upper().startswith("CORE")) and (daily_plan_sheet.iloc[i]['Planning Status'].upper() == 'PLANNED')):
                    execution_date.append((daily_plan_sheet.iloc[i]['Execution Date']))
                    maintenance_window.append(daily_plan_sheet.iloc[i]['Maintenance Window'])
                    mpbn_cr_no.append(daily_plan_sheet.iloc[i]['CR NO'])
                    cr_category.append(category)
                    ...
                    impacted_node_details.append(daily_plan_sheet.iloc[i]['IMPACTED NODE'])
                    Kpis_to_be_monitored.append(daily_plan_sheet.iloc[i]['KPI DETAILS'])
            dictionary2 = {'CR': mpbn_cr_no, 'Maintenance Window': maintenance_window, 'CR Category': cr_category, 'Impact': impact, 'Location': location, 'Circle': circle,
                           'MPBN Activity Title': mpbn_activity_title, 'CR Owner Domain': cr_owner_domain, 'Change Responsible': mpbn_change_responsible_executor,
                           'Technical Validator/Team Lead': validator, 'InterDomain': inter_domain, 'Impacted Node Details': impacted_node_details, 'KPIs to be monitored': Kpis_to_be_monitored}
            ...
            dictionary3 = {'CR': mpbn_cr_no, 'Maintenance Window': maintenance_window, 'CR Category': cr_category, 'Impact': impact, 'Location': location, 'Circle': circle,
                           'MPBN Activity Title': mpbn_activity_title, 'CR Owner Domain': cr_owner_domain, 'Change Responsible': mpbn_change_responsible_executor,
                           'Technical Validator/Team Lead': validator, 'InterDomain': inter_domain, 'Impacted Node Details': impacted_node_details, 'KPIs to be monitored': Kpis_to_be_monitored,
            ...
            dictionary4 = {'CR': mpbn_cr_no, 'Maintenance Window': maintenance_window, 'CR Category': cr_category, 'Impact': impact, 'Location': location, 'Circle': circle,
                           'MPBN Activity Title': mpbn_activity_title, 'CR Owner Domain': cr_owner_domain, 'Change Responsible': mpbn_change_responsible_executor,
                           'Technical Validator/Team Lead': validator, 'InterDomain': inter_domain, 'Impacted Node Details': impacted_node_details, 'KPIs to be monitored': Kpis_to_be_monitored}
            ...
            if ((len(df) > 0) or (len(df2) > 0) or (len(df3) > 0) or (len(df4) > 0)):
                ...
                response = messagebox.askokcancel("  No Interdomain Data Present!",
                                                  "No Interdomain detected in the 'Domain kpi' column of the uploaded planning sheet, Kindly check! if this is ok, Press Ok!")
    ...
    # Exception for condition when Today's maintenance date is not present.
        ...
        messagebox.showerror("  Data for today's maintenance not found", str(error))
    ...
    # Handling Key Error
        ...
        messagebox.showerror("  Check for the below Header(KeyError) ", str(e))
...
# paco_cscore("Arka Maiti",r"C:/Users/emaienj/Downloads/New_folder_(3)/MPBN Daily Planning Sheet.xlsx")
